...
const controlador = 'Admin/Ctrl_edmu/';
...
function datosTabla(data) {
    ...
    if (response.registros === 0) {
        ...
        $.each(response.datos, function (i, item) {
            ...
            html += '<tr>';
            html += '<td>' + item.id_edmu + '</td>';
            html += '<td>' + item.edmu + '</td>';
...
function Editar(id, edmu, detalle, estado) {
    invisible('alerta');
    $('#id_edmu').val(id);
    $('#edmu').val(edmu);
    $('#detalle').val(detalle); 
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            switch (response.estatus) {
                ...
                case 401:
                    var mensaje = 'El Edmu "' + response.datos + '" ya esta registrado';
